...
contract OptionVault is ERC721, Ownable {
    ...
    constructor() ERC721("Option", "OPT") {
        ...
        USDC_Contract = IERC20(USDC_Address);
    }
    ...
    //         require(USDC_Contract.approve(msg.sender, address(this)), "You must approve the contract as a receiver for the required USDC");

    //         // Check if the user has sent enough USDC in the transaction to cover the collateral requirement
    //         require(USDC_Contract.transferFrom(msg.sender, address(this), strikePrice * underlyingValue), "You must send enough USDC to cover the collateral requirement for this put option");
    ...
    // function exercisePutOption(uint256 _optionId) public returns(string memory) {
    ...
    //     require(IERC20(USDC_Contract).transfer(option.counterpartyAddress, usdcPrice), "USDC transfer failed");

    //     // Transfer the underlying value (eth) to the buyer/holder of the option NFT
    //     uint256 underlyingValue = option.underlyingValue;
    //     // require(payable(msg.sender).transfer(msg.sender, underlyingValue), "Underlying asset transfer failed");
    ...
    function expireOption(
        ...
        require(!isExecutable(option), "Cannot expire an option that is still executable");
